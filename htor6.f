...
* Compute Chern number using HF!!
* Assume AF case at half-filling i.e rmu = 0 (no need to 
* determine s.c). Determine however the magnetization rmom.
* Chanell 29 (fort.29) is sometimes used for output.

      implicit real*8 (a-h,o-z)
      parameter (maxp=5500,kdim=4,nband=2)
...
* A.
      double precision nosA_up(maxp),dosA_up(maxp)
      double precision nosA_do(maxp),dosA_do(maxp)
      double precision tnosA(maxp),tdosA(maxp)
* B.
      double precision nosB_up(maxp),dosB_up(maxp)
      double precision nosB_do(maxp),dosB_do(maxp)
      double precision tnosB(maxp),tdosB(maxp)
* Total.
      double precision tnos(maxp),tdos(maxp)

      double precision rw5(maxp)
      double precision rw6(maxp)
      double precision rw7(maxp)

      double precision x1(2),f1(2)
      double precision emin,emax,ebot,etop,e(4,8),ee(4),bb(4)
     ...
     .                 v2(maxp),v2d(maxp),rgamma(maxp)
      ...
      double precision h1(4,4,2), o1(4,4,2), z1(4,4,2), w1(4,11), 
     .                 e1(4)

      double precision e1xp(4)  !kx+d
      double precision e1xm(4)  !kx-d
      double precision e1yp(4)  !ky+d
      double precision e1ym(4)  !ky-d

      double precision z1xp(4,4,2)  !kx+d
      double precision z1xm(4,4,2)  !kx-d
      double precision z1yp(4,4,2)  !ky+d
      double precision z1ym(4,4,2)  !ky-d
      double precision phiC(maxp), tx(2), ty(2), ekn(4)

      complex*16 H0, Hx, Hy, Hz
      complex*16 expphi, ctemp1, ctemp2


      complex*16 b(4,4,8), xi,  csum11, csum12, csum21, csum22,
     .           csum, csum1, csum2, csum3, csumx, csumy
      complex*16 csum5, csum6, csum7
      complex*16 ukxp(4,2)
      complex*16 ukxm(4,2)
      complex*16 ukyp(4,2)
      complex*16 ukym(4,2)
      complex*16 derxuk(4,2)
      complex*16 deryuk(4,2)
      complex*16 detS
      complex*16 Axp(2,2)
      complex*16 Axm(2,2)
      complex*16 Ayp(2,2)
...
* Open files.
      open(unit=1,file='hmf1.d',status='OLD')
      open(unit=2,file='band_mf1.d',status='UNKNOWN')
      open(unit=7,file='phase_up.d',status='UNKNOWN')
      open(unit=8,file='phase_do.d',status='UNKNOWN')
      open(unit=9,file='chern-mf.d',status='UNKNOWN')

      open(unit=11,file='dosnos_Aup.d',status='UNKNOWN')
      open(unit=12,file='dosnos_Ado.d',status='UNKNOWN')
      open(unit=13,file='dosnos_Bup.d',status='UNKNOWN')
      open(unit=14,file='dosnos_Bdo.d',status='UNKNOWN')
      open(unit=15,file='dosnosA.d',status='UNKNOWN')
      open(unit=16,file='dosnosB.d',status='UNKNOWN')
      open(unit=17,file='dosnos.d',status='UNKNOWN')
...
* For DOS.
*      voldos =  ( 1.d0/(nkabc(1)*nkabc(2)*nkabc(3)) )*(1.d0/6.d0)
      voldos =   ( 1.d0/((nkabc(1)+1)*(nkabc(2)+1)) )*(1.d0/6.d0)
*      voldos = voldos/2.d0    !One electron per band
*      voldos = voldos/2.d0    !One electron per band
...
* Loop over Hubbard U and staggered potential.
      do iu = 1,nhubb
      if(iread.eq.0) uhubb = ustart+dble(iu-1)*deltaU
      do if = 1,nstagg
      if(iread.eq.0) deltaAB  = deltastart + dble(if-1)*ddelta


* Guess moment.
*      rmom0   = 0.5d0
*      sfield  = uhubb*rmom0/2.d0
...
* Loop over site A and B and spin up and down !!
* ipp = 1 i.e A up.
* ipp = 2 i.e A do.
* ipp = 3 i.e B up.
* ipp = 4 i.e B do.
...
* Find rmu - should be zero in the paricle-hole symmetric case phi=pi/2 !!
...
* Diagonal elements H11 and H22 first!
      ...
      h1(1,1,1)  =  -2.d0*t2*(rtemp1 + rtemp2 + rtemp3) + eA + sfield   !A up 
      h1(2,2,1)  =  -2.d0*t2*(rtemp1 + rtemp2 + rtemp3) + eA - sfield   !A do 

      rtemp1     = dcos(dsqrt(3.d0)*kx + phi)
      rtemp2     = dcos(-dsqrt(3.d0)*kx/2.d0 + 3.d0*ky/2.d0 + phi)
      rtemp3     = dcos(-dsqrt(3.d0)*kx/2.d0 - 3.d0*ky/2.d0 + phi)

      h1(3,3,1)  =  -2.d0*t2*(rtemp1 + rtemp2 + rtemp3) + eB - sfield   !B up
      h1(4,4,1)  =  -2.d0*t2*(rtemp1 + rtemp2 + rtemp3) + eB + sfield   !B do 
...
* Hopping H12.
      ...
      h1(1,3,1)  =  -t1*(rtemp1 + rtemp2 + rtemp3)
      h1(2,4,1)  =  -t1*(rtemp1 + rtemp2 + rtemp3)

      rtemp1     = dsin(ky)
      rtemp2     = dsin(-dsqrt(3.d0)*kx/2.d0 - ky/2.d0)
      rtemp3     = dsin( dsqrt(3.d0)*kx/2.d0 - ky/2.d0)

      h1(1,3,2)  =   t1*(rtemp1 + rtemp2 + rtemp3)
      h1(2,4,2)  =   t1*(rtemp1 + rtemp2 + rtemp3)
...
* For kx,ky!!
      ...
      if(dabs(rmu).gt.0.000001d0) stop 'Error in rmu !!'
*      write(6,*)'phi mu e1 e2 gap:' 
*      write(6,'(7f10.6)')phi/pi, rmu, e1(1),e1(2),e1(3),e1(4),gap 
*      stop
***

* Do LDOS for this choice of uhubb and deltaAB !!
      ux=-dux
...
* Construct LDOS. Sum over the ll=4 bands here and choosen orbital ipp.
      do ll = 1, kdim
      do it = 1, 6
      do ie = 1, 4
      ee(ie)=e(ll,p(ie,it))
      bb(ie)=dreal(b(ipp,ll,p(ie,it)))*dreal(b(ipp,ll,p(ie,it))) +
     .       dimag(b(ipp,ll,p(ie,it)))*dimag(b(ipp,ll,p(ie,it)))
      enddo
      wgt = (bb(1)+bb(2)+bb(3)+bb(4))*voldos/4
      ebot=dmin1(ee(1),ee(2),ee(3),ee(4))
      etop=dmax1(ee(1),ee(2),ee(3),ee(4))
      if(ebot .lt. emax)then
      call slinz1(wgt,ee,emin,emax,nos,npts)
      if(etop .gt. emin)
     .call sliny1(wgt,ee,emin,emax,dos,npts)
      endif
      ...
      nos(i) = 0.d0
      dos(i) = 0.d0
      tnosA(i)   = nosA_up(i)+nosA_do(i)
      tdosA(i)   = dosA_up(i)+dosA_do(i)
      enddo
      endif
      if (ipp.eq.3) then
      do i =1,npts
      ...
      nos(i) = 0.d0
      dos(i) = 0.d0
      tnosB(i)   = nosB_up(i)+nosB_do(i)
      tdosB(i)   = dosB_up(i)+dosB_do(i)
      tdos(i)    = tdosA(i)+tdosB(i)
      tnos(i)    = tnosA(i)+tnosB(i)
      enddo
      endif
...
* Fix rmu=0 always corresponds to ief=npts/2.
* A atom.
      rnos_up = nosA_up(npts/2)
      rnos_do = nosA_do(npts/2)
      sumA    = rnos_do + rnos_up
      if(iread.eq.1)write(6,'(a15,(3f12.6))')'NOS atom A:',
     .              rnos_up, rnos_do, sumA
      rmom    = rnos_up - rnos_do
      rmomA   = rmom 
      if(iread.eq.1)write(6,'(a25,(f12.6))')'Moment from NOS (A):',rmom
* B atom.
      rnos_up = nosB_up(npts/2)
      rnos_do = nosB_do(npts/2)
      sumB    = rnos_do + rnos_up
      rmomB   = rnos_up - rnos_do 
      if(iread.eq.1)write(6,'(a15,(3f12.6))')'NOS atom B:',
     .              rnos_up, rnos_do, sumB
      if(iread.eq.1)write(6,'(a25,(f12.6))')'Moment from NOS (B):',
     .rmomB
      if(iread.eq.1)write(6,'(a25,(f12.6))')'Electrons per cell:',
     .sumA+sumB
      if(iread.eq.1.and.dabs(sumA+sumB-2.d0).gt.0.00000001d0) 
     .                  stop 'Error in number electrons !!'
      if(iread.eq.1.and.dabs(rmomA+rmomB).gt.0.00000001d0) 
     .                  stop 'Error in moments !!'


* Compare input moment with calculated and then mix!!
*      rsum2   = dabs(rmom0-rmom)
      rsum2   = dabs(rmom0-rmomB)
      rmom  = alpha*rmom0 + (1.d0-alpha)*rmomB

* If converged !!
      if(rsum2.lt. conv2)then
      write(6,*)'Magnetic moment converged!!!'
      write(6,*)'Iteration:',initer
      write(6,'(a30,(f12.6))')'Magnetic moment:',rmom0
      write(29,'(3f12.6)') rmom, uhubb, gap
      write(6,'(a30,(f12.6))')'Staggered density (A-B):',
     .                         sumA-sumB

* If no loop in U we write and do bands !!
      if(iread.eq.1)then
      do i =1,npts
      write(11,'(3f12.6)') freq(i),dosA_up(i),nosA_up(i)
      write(12,'(3f12.6)') freq(i),dosA_do(i),nosA_do(i)
      write(13,'(3f12.6)') freq(i),dosB_up(i),nosB_up(i)
      write(14,'(3f12.6)') freq(i),dosB_do(i),nosB_do(i)
      write(15,'(3f12.6)') freq(i),tdosA(i),tnosA(i)
      write(16,'(3f12.6)') freq(i),tdosB(i),tnosB(i)
      write(17,'(3f12.6)') freq(i),tdos(i),tnos(i)
      enddo
...
789   format(14f15.6)
      if(ix.eq.1) write(6,*)' Bands G-K:'

      write(2,789)dsqrt(kx**2 + ky**2)/twopi,e1(1),e1(2),e1(3),e1(4)
...
* H21.
      ...
      if(ix.eq.1) write(6,*)' Bands K-Kprim:'
      write(2,789)kmaxGK + dble(ix-1)*dsqrt(dkx**2 + dky**2)/twopi,
     .             e1(1),e1(2),e1(3),e1(4)
...
* H21.
      ...
      if(ix.eq.1) write(6,*)' Bands Kprim-G:'
      write(2,789)kmaxKKP + dble(ix-1)*dsqrt(dkx**2 + dky**2)/twopi,
     .             e1(1),e1(2),e1(3),e1(4)
...
* End k-loops
      ...
      if(iread.eq.1)then
      write(6,*)'Iteration:',initer
      write(6,'(a30,(f12.6))')'Difference in magnetic moment:',rsum2
      write(6,'(a30,(2f12.6))')'gap and rmom*U:',gap, rmom*uhubb
      write(6,'(a15,(2f12.6))')'Old and new:',rmom0,rmom
      endif
...
* For kx,ky!!
      ...
      if(ik.ne.(nkabc(1)+1)*(nkabc(2)+1))then
      write(6,*)
      write(6,*)'Wrong k-points for band-energy!!!'
      write(6,*)
      endif

      ikband = ik
      eband = eband/dble(ikband)

* Note that 1.d0 here means filling squared !!
      ecorr = -uhubb*(1.d0-rmom*rmom)/2.d0
      etot  = ecorr + eband

      write(6,*)
      write(6,*)'2D hexagonal plane!'
      write(6,'(a29,f10.3)')'Fermi energy (eV):',rmu
      write(6,'(a29,f10.3)')'Hubbard U (eV):',uhubb
      write(6,'(a29,f10.2)')'Magnetic moment:',rmom
      write(6,'(a29,f10.3)')'Band energy (eV):',eband
      write(6,'(a29,f10.3)')'Corr. energy (eV):',ecorr
      write(6,'(a29,f10.3)')'Total energy (eV):',etot

* Band-energy from DOS!!
      ief      = npts/2
      do   iw  = 1,ief
      rw5(iw)  = freq(iw)*(tdosA(iw)+tdosB(iw))
      rw6(iw)  = tdosA(iw) 
      rw7(iw)  = tdosB(iw) 
      enddo

      diw   = freq(2)-freq(1)
      tem1  = trapez(rw5,diw,ief)

      tem2  = trapez(rw6,diw,ief)
      tem3  = trapez(rw7,diw,ief)

      write(6,*)
      write(6,'(a32,f10.3)')'Band energy from NOS:',tem1
      write(6,'(a32,f10.3)')'Filling atom A from integrating DOS:',tem2
      write(6,'(a32,f10.3)')'Filling atom B from integrating DOS:',tem3
      write(6,*)

      write(6,*)
      write(6,'(a27,f10.3)')'Band energy (AF) (eV):',eband
      write(6,'(a27,f10.6)')'sfield:',sfield
      write(6,'(a27,f10.6)')'rmom:',rmom
      write(6,*)

      goto 5656 

* For s.c rmom0 do Chern number !!
...
* Determine dual state! Band 1 and 2 occupied.
      csum11 = dcmplx(0.d0,0.d0)
      csum12 = dcmplx(0.d0,0.d0)
      csum21 = dcmplx(0.d0,0.d0)
      csum22 = dcmplx(0.d0,0.d0)

      do i1 = 1,kdim

      csum11 = csum11 + dcmplx(z1  (i1,1,1),-z1  (i1,1,2))*   !11 element of S-matrix
     .                  dcmplx(z1xp(i1,1,1), z1xp(i1,1,2))

      csum12 = csum12 + dcmplx(z1  (i1,1,1),-z1  (i1,1,2))*   !12 element
     .                  dcmplx(z1xp(i1,2,1), z1xp(i1,2,2))

      csum21 = csum21 + dcmplx(z1  (i1,2,1),-z1  (i1,2,2))*   !21 element
     .                  dcmplx(z1xp(i1,1,1), z1xp(i1,1,2))

      csum22 = csum22 + dcmplx(z1  (i1,2,1),-z1  (i1,2,2))*   !22 element
     .                  dcmplx(z1xp(i1,2,1), z1xp(i1,2,2))

      enddo


* Inverse of S is needed! Ceresoli, PRB74 024408 (2006) Eq. A11-12.
      detS = csum11*csum22 - csum12*csum21
...
*      ukxp(i1,1) = dcmplx(z1xp(i1,1,1),z1xp(i1,1,2))*( csum22/detS )  !Band 1
*     .           + dcmplx(z1xp(i1,2,1),z1xp(i1,2,2))*(-csum21/detS )
*
*      ukxp(i1,2) = dcmplx(z1xp(i1,1,1),z1xp(i1,1,2))*(-csum12/detS )  !Band 2
*     .           + dcmplx(z1xp(i1,2,1),z1xp(i1,2,2))*( csum11/detS )

* Limit dx and dy to zero !!
      ukxp(i1,1) = dcmplx(z1xp(i1,1,1),z1xp(i1,1,2))/csum11
      ukxp(i1,2) = dcmplx(z1xp(i1,2,1),z1xp(i1,2,2))/csum22
...
* Determine dual state! Band 1 and 2 occupied.
     ...
     .                  dcmplx(z1xm(i1,1,1), z1xm(i1,1,2))
      ...
      csum12 = csum12 + dcmplx(z1  (i1,1,1),-z1  (i1,1,2))*   !22 element
...
* Invers of S is needed! Ceresoli, PRB74 024408 (2006) Eq. A11-12.
      detS = csum11*csum22 - csum12*csum21
...
*      ukxm(i1,1) = dcmplx(z1xm(i1,1,1),z1xm(i1,1,2))*( csum22/detS )  !Band 1
*     .           + dcmplx(z1xm(i1,2,1),z1xm(i1,2,2))*(-csum21/detS )
*
*      ukxm(i1,2) = dcmplx(z1xm(i1,1,1),z1xm(i1,1,2))*(-csum12/detS )  !Band 2
*     .           + dcmplx(z1xm(i1,2,1),z1xm(i1,2,2))*( csum11/detS )

* Limit dx and dy to zero !!
      ukxm(i1,1) = dcmplx(z1xm(i1,1,1),z1xm(i1,1,2))/csum11
      ukxm(i1,2) = dcmplx(z1xm(i1,2,1),z1xm(i1,2,2))/csum22
...
*      ukyp(i1,1) = dcmplx(z1yp(i1,1,1),z1yp(i1,1,2))*( csum22/detS )  !Band 1
*     .           + dcmplx(z1yp(i1,2,1),z1yp(i1,2,2))*(-csum21/detS )
*
*      ukyp(i1,2) = dcmplx(z1yp(i1,1,1),z1yp(i1,1,2))*(-csum12/detS )  !Band 2
*     .           + dcmplx(z1yp(i1,2,1),z1yp(i1,2,2))*( csum11/detS )

* Limit dx and dy to zero !!
      ukyp(i1,1) = dcmplx(z1yp(i1,1,1),z1yp(i1,1,2))/csum11
      ukyp(i1,2) = dcmplx(z1yp(i1,2,1),z1yp(i1,2,2))/csum22
...
* Invers of S is needed! Here only 2 by 2 so
* do it analytically. Ceresoli, PRB74 024408 (2006) Eq. A11-12.
      detS = csum11*csum22 - csum12*csum21
...
*      ukym(i1,1) = dcmplx(z1ym(i1,1,1),z1ym(i1,1,2))*( csum22/detS )  !Band 1
*     .           + dcmplx(z1ym(i1,2,1),z1ym(i1,2,2))*(-csum21/detS )
*
*      ukym(i1,2) = dcmplx(z1ym(i1,1,1),z1ym(i1,1,2))*(-csum12/detS )  !Band 2
*     .           + dcmplx(z1ym(i1,2,1),z1ym(i1,2,2))*( csum11/detS )

* Limit dx and dy to zero !!
      ukym(i1,1) = dcmplx(z1ym(i1,1,1),z1ym(i1,1,2))/csum11
      ukym(i1,2) = dcmplx(z1ym(i1,2,1),z1ym(i1,2,2))/csum22

      enddo


* Determine the covariant-derivatives!
      do ib      = 1,nband    !Band index 1 and 2
      do i2      = 1,kdim
      ...
      do i2 = 1,kdim
      C1 = C1  +    dconjg(derxuk(i2,1))*deryuk(i2,1)
     .         -    dconjg(deryuk(i2,1))*derxuk(i2,1)
      C2 = C2  +    dconjg(derxuk(i2,2))*deryuk(i2,2)
     .         -    dconjg(deryuk(i2,2))*derxuk(i2,2)
      enddo
      ...
      write(6,'(a30,3f10.2)')'Chern number band 1:'
      write(6,'(5f8.2,4x,f8.2)')C1*volwgt*xi/twopi
      write(6,'(a30,3f10.2)')'Chern number band 2:'
      write(6,'(5f8.2,4x,f8.2)')C2*volwgt*xi/twopi
      write(6,'(a30,3f10.2)')'Chern (total):'
      write(6,'(5f8.2,4x,f8.2)')(C1+C2)*volwgt*xi/twopi

*      write(29,'(3f12.2)') dreal( (C1+C2)*volwgt*xi/twopi ), uhubb,
*     .                     rmom


5656  continue

      enddo    !Loop U and staggered potential
      enddo
...
***
      double precision function trapez (f,h,n)
c 93.05.18 trapezoidal integration = S[i] f(i)
c h = mesh size
      implicit real*8 (a-h,o-z)
      dimension f(n)
      trapez     = .5d0*(f(1)+f(n))
...
***
      subroutine sliny1(volwgt,ecorn,emin,emax,dos,npts)
...
C Local variables:
      integer i,i0(4),m
      double precision d1mach,de,e,e1,e2,e3,e4,c0,c1,c2,c3,x
C External calls:
...
C ----sort ecorn -------
ckk e1 smallest!
cck already ordered from slinz1.f!!
...
11    format(4f10.6)
      ...
      do m=1,4
        ...
        elseif (ecorn(m).gt.emax) then
          ...
          i0(m) = 2 + int((ecorn(m)-emin)/de )
c         i0(m) = 2 + int((ecorn(m)-emin)/de - d1mach(3))
        endif
      enddo

ckk      write(6,12)i0(1),i0(2),i0(3),i0(4)
12    format(4i5)
ckk      write(6,*)
      ...
      subroutine slinz1(volwgt,ecorn,emin,emax,nos,npts)
...
C Local variables:
      integer i,i0(4),m
      double precision d1mach,de,e,e1,e2,e3,e4,c0,c1,c2,c3,x,x0
C External calls:
      external d1mach,dshell
C Intrinsic functions:
...
c           i0(m) = 2 + int((ecorn(m)-emin)/de - d1mach(3))
      ...
      subroutine sort(m,n,darray,br,rslask,bi,cslask)
C- shell sort of a array of integer vectors
C ----------------------------------------------------------------------
Ci Inputs:
Ci   m     :number of components in darray
Ci   n     :number of elements in darray
Cio  darray:array to be sorted
Co Outputs:
Cio  darray:array to be sorted
C ----------------------------------------------------------------------
      implicit none
C Passed variables:
      integer m,n,i1
      double precision darray(m,0:n-1),br(n,n),bi(n,n)
C Local variables:
      integer lognb2,i,j,k,l,n2,nn,mm,mmm
      double precision dt,rslask(n),cslask(n)
C Intrinsic functions:
    ...
    3     continue
          l = i + n2
          do  15  mm = 1, m
            if (darray(mm,l) - darray(mm,i)) 16,15,11
   16       continue

            do mmm = 1, m

              dt = darray(mmm,i)
              darray(mmm,i) = darray(mmm,l)
              darray(mmm,l) = dt
ckk Sort also eigenvectors!
*              do i1 =  1,14
              do i1 =  1,n
              rslask(i1) = br(i1,i+1)
              cslask(i1) = bi(i1,i+1)
...
C Passed variables:
      integer m,n,il(n)
      double precision darray(m,0:n-1)
C Local variables:
      integer lognb2,i,j,k,l,n2,nn,mm,mmm
      double precision dt
C Intrinsic functions:
